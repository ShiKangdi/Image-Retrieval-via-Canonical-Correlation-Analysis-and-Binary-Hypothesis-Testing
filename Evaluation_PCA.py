...
Xor = np.dot(np.transpose(T_Max_Rshape_p5),T_Max_Rshape_p5)
Xor_R = (Xor + np.transpose(Xor))/(2*K)
Eigval,Eigvec = eig(Xor_R)
...
if __name__ == '__main__':
    ...
    for n_comp in e:    
        ...
        for q in range(55):
            ...
            if not os.path.exists("./Test_results/Oxford/PCA/" + METHOD  + str(n_comp)):
                os.makedirs("./Test_results/Oxford/PCA/" + METHOD  + str(n_comp))            
            text_file = open("./Test_results/Oxford/PCA/" + METHOD  + str(n_comp) + "/Query"+str(q)+ ".txt","w")
